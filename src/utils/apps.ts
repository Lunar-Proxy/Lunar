interface App {
  ...
  function createAppCard(container: HTMLElement, { icon, website, name, error }: App): void {
    ...
    appName.className = "text-lg text-white font-semibold app-name text-center mt-2";
  ...
  function filterApps(): void {
    ...
    document.querySelectorAll<HTMLElement>(".app-card").forEach((card) => {
      const appName = (card.querySelector(".app-name") as HTMLElement).innerText.toLowerCase();
